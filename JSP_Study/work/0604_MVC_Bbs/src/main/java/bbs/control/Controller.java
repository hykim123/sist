...
private static final long serialVersionUID = 1L;
	...
	public void init() throws ServletException {
		...
		//절대경로화 시킨 이유는
		//해당 파일의 내용(클래스 경로)을 스트림을 이용하여 읽어와서
		//Properties객체에 담기 위함이다.
		Properties prop = new Properties();
		
		//Properties의 load함수를 이용하여 내용들을 읽기한다.
		//이때 필요한 객체가 InputStream이다.
		FileInputStream fis = null;
		try {
			//action.properties 파일과 연결되는 스트림 준비
			fis = new FileInputStream(realPath);
			
			prop.load(fis); //action.properties파일의 내용들을 읽어서 비어있던 Properties객체에 키와 값을 쌍으로 저장했다.
		...
		//actionMap에 저장해야 한다.
		Iterator<Object> it = prop.keySet().iterator(); //키들만 넘어옴
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 사용자가 무엇을가지고 어떤처리를 원하는지..
		// 요청시 한글처리
		request.setCharacterEncoding("utf-8");
		
		//type이라는 파라미터 받기
		String type = request.getParameter("type"); // 이 타입에 emp가 들어가있으면 emp선택해서 처리해야함
		
		//type이 null 이면 기본객체 인식할 수 있도록 초기값 넣기
		if(type == null) 
			type = "index";
